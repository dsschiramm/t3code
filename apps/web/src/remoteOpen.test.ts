...
describe("resolveRemoteOpenState", () => {
  ...
  it("uses deep links for a primary target reached over the network", () => {
    expect(
      resolveRemoteOpenState({
        target: primaryTarget("https://sol.example.net"),
        sshAlias: null,
        isDesktopRenderer: false,
        remoteOpenTargets: REMOTE_OPEN_TARGETS,
      }),
...
describe("buildRemoteOpenUrl", () => {
  it("builds a vscode-remote deep link", () => {
    expect(
      buildRemoteOpenUrl({
        editor: "vscode",
        host: "sol.example.net",
        absolutePath: "/home/theo/code/my repo",
      }),
    ).toBe("vscode://vscode-remote/ssh-remote+sol.example.net/home/theo/code/my%20repo");
  });
  ...
  it("builds Zed's ssh deep link", () => {
    ...
    ).toBe("zed://ssh/sol.example.net/home/theo/code/my%20repo");
  });
